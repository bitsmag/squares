...
describe('Socket Integration', function() {
  ...
  it('connects to matchSockets and receives connectedPlayers', function(done){
    ...
    const hostClient = ioClient.connect(`http://localhost:${port}/createMatchSockets`, { transports: ['websocket'] });
    hostClient.on('connect', function(){
      ...
      const client = ioClient.connect(`http://localhost:${port}/matchSockets`, { transports: ['websocket'] });
