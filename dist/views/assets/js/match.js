...
matchSockets.on('connect', function () {
    ...
    matchSockets.on('playerConnected', function (_connectedPlayer) {
        $('#connectionList').append($('<li>').text(_connectedPlayer.playerName + ' connected to the match'));
    });
    matchSockets.on('playerDisconnected', function (_disconnectedPlayer) {
        $('#connectionList').append($('<li>').text(_disconnectedPlayer.playerName + ' disconnected from the match'));
    });
...
function matchEnd() {
    let congratsDiv = '<div id="congratsDiv" class="form-style-8"><h2>Congratulations ' +
